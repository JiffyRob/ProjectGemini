...
class Space(game_state.GameState):
    ...
    def update(self, dt):
        ...
        rot_speed = 0.25
        if keys[pygame.K_UP]:
            self.camera.rotation *= util3d.Quaternion(dt * rot_speed, (1, 0, 0))
        if keys[pygame.K_DOWN]:
            self.camera.rotation *= util3d.Quaternion(-dt * rot_speed, (1, 0, 0))
        if keys[pygame.K_LEFT]:
            self.camera.rotation *= util3d.Quaternion(-dt * rot_speed, (0, 1, 0))
        if keys[pygame.K_RIGHT]:
            self.camera.rotation *= util3d.Quaternion(dt * rot_speed, (0, 1, 0))
        if keys[pygame.K_LCTRL]:
            self.camera.rotation *= util3d.Quaternion(dt * rot_speed, (0, 0, 1))
